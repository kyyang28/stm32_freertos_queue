...
void task1_task(void *pvParameters)
{
    uint8_t i = 0;
    BaseType_t err;
    bool isUserBtnPressed = false;
    
    while (1) {
        isUserBtnPressed = button.dev.btnPressed;
        
        if ((KeyQueue != NULL) && (isUserBtnPressed)) {
            err = xQueueSend(KeyQueue, &isUserBtnPressed, 10);
            
            if (err == errQUEUE_FULL) {
                printf("Key queue is full, failed to send the data!\r\n");
            }
        }
...
void keyprocess_task(void *pvParameters)
{
    ...
    while (1) {
        if (KeyQueue != NULL) {
            /* Request data from KeyQueue */
            if (xQueueReceive(KeyQueue, &isBtnPressed, portMAX_DELAY)) {
