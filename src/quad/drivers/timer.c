...
void configTimeBaseKhz(TIM_TypeDef *tim, uint16_t period, uint8_t khz)
{
	...
	TIM_TimeBaseStructure.TIM_ClockDivision = 0;                // 0 = TIM_CKD_DIV1
...
void timerInit(void)
{
	...
	for (int timerIndex = 0; timerIndex < USABLE_TIMER_CHANNEL_COUNT; timerIndex++) {
		...
		if (timerHardwarePtr->usageFlags == TIM_USE_PWM) {			// if timer uses PWM input capture mode, initialise to push-down configuration
...
void timerNVICConfigure4FreeRTOS(uint8_t irq, uint8_t PremptPrio, uint8_t SubPrio)
{
	NVIC_InitTypeDef NVIC_InitStructure;
	
//	printf("irq: %u, %s, %d\r\n", irq, __FUNCTION__, __LINE__);		// irq (TIM5_IRQn) = 50
	NVIC_InitStructure.NVIC_IRQChannel = irq;
	NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = PremptPrio;
...
//        if (tim == TIM3) {
//            printf("TIM3 is updating.....\r\n");
//        } else if (tim == TIM4) {
//            printf("TIM4 is updating.....\r\n");
//        }
        
//        if (tim == TIM2) {
//            printf("TIM2 is updating....\r\n");
//        }
